...
export function AdminUsersPage() {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      toast.error('임직원 목록을 불러오지 못했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1>임직원 관리</h1>
          <p className="mt-1 text-muted-foreground">임직원 계정과 포인트 운영 상태를 확인하세요.</p>
        </div>
        <button
          type="button"
      ...
      {loading ? (
        <div className="flex min-h-[400px] items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-muted-foreground">임직원 정보를 불러오는 중입니다.</p>
          </div>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/50 p-4">
          <div className="w-full max-w-md rounded-[var(--radius-card)] bg-card p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              재직 중인 임직원 전체에게 300,000P를 일괄 지급합니다.
            </p>

            <div className="mt-5 space-y-3">
              {[
                { label: '지급 대상', value: '재직 임직원 전체' },
